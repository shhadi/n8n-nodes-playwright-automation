...
export class PageController implements IController {
	async execute(
		...
		if (!session) {
			throw new NodeOperationError(executeFunctions.getNode(), `Session ${sessionId} not found`, {
				itemIndex,
			});
		}
		...
		switch (operation) {
			case PageOperation.Navigate:
				result = await navigate(
					page,
					executeFunctions.getNodeParameter('url', itemIndex) as string,
				);
				break;
			...
			case PageOperation.Fill:
				result = await type(
					page,
					executeFunctions.getNodeParameter('selector', itemIndex) as string,
					executeFunctions.getNodeParameter('value', itemIndex) as string,
				);
				break;
			case PageOperation.Wait: {
				...
				if (waitType === WaitType.Time) {
					result = await wait(
						page,
						WaitType.Time,
						executeFunctions.getNodeParameter('waitTime', itemIndex) as number,
					);
				} else {
					result = await wait(
						page,
			...
			case PageOperation.GetAttribute:
				result = await getAttribute(
					page,
					executeFunctions.getNodeParameter('selector', itemIndex) as string,
					executeFunctions.getNodeParameter('attributeName', itemIndex) as string,
				);
				break;
			case PageOperation.Count:
				result = await countElements(
					page,
					executeFunctions.getNodeParameter('selector', itemIndex) as string,
				);
				break;
			...
			case PageOperation.PDF: {
				...
				const landscape = executeFunctions.getNodeParameter('landscape', itemIndex) as boolean;
				const printBackground = executeFunctions.getNodeParameter(
					'printBackground',
					itemIndex,
				) as boolean;

				const buffer = await generatePDF(page, { format, landscape, printBackground });
				const binaryData = await executeFunctions.helpers.prepareBinaryData(
					buffer,
					'document.pdf',
					'application/pdf',
			...
			default:
				throw new NodeOperationError(
					...
					`Operation ${operation} not supported for Page resource`,
					{ itemIndex },
				);
		}
